...
bot.on('message', async (msg) => {
  ...
  if (txt === 'инфо') {
    return sendSafe(msg,
      ...
      `👤 Автор: Errnick_\n` +
      `📦 Версия: 1.0.3\n` +
      `💬 Telegram: <a href="https://t.me/Errnick_code">Связь и исходный код</a>`,
      { parse_mode: 'HTML' }
    );
  }
  ...
  // --- Список команд (подсказка) ---
  if (!command) {
    const cmds = [
      'заявка(без !EC) - создать заявку',
      'админы - список админов',
      'сайт - перейти в магазин/донат',
      'мой ник(без !EC) - показать ваш ник',
      'ник(без !EC, в ответ на сообщение) - показать ник другого игрока',
      'админ add|remove - добавить или удалить админа [только админ]',
      'список - показать whitelist [только админ]',
      'убрать <ник> - удалить игрока [только админ]',
      'сборка - отправить файл сборки [только админ]',
      'команда - выполнить RCON команды [только админ]',
      'добавить - добавить игрока без заявки [только админ]',
      'удалить - удалить игрока из базы и whitelist [только админ]'
    ];
    return sendSafe(msg, `📜 Доступные команды через !EC:\n${cmds.join('\n')}`);
  }
